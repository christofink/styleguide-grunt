	...
	$sgViewport.load(function (){
		...
		$('#sg-t-annotations').on("click", function(e){
			...
			if($vp.find('.sg-annotations').length==0) {
				...
				//var comments = annotations.comments || { title: "", comment: "" }; //Loaded from an external file annotations.js //Todo - Load from json do not store in global namespace
			
				buildAnnotationView(comments);
			} else {
				$annotations.toggle();
			}
		});
		...
		function buildAnnotationView(data) {
			$sgPattern.each(function(index) { //Loop through each pattern
				
				var item = data[index];
				var sAnnotation = "<h2>"+ item.el + "</h2><h3>" + item.title + "</h3><hr/><p>" + item.comment + "</p>";
				
				$this = $(this),
				$thisAnnotation = sAnnotation; 
...
/**
 * Global navigation
 * Todo - Need to work in Older browsers!!!
 */
(function(window, document, undefined) {
     ...
     * @private
     */
    function _updateIframeSource(url) {
     ...
     * @private
     */
    function _formatHashToURL(hash) {
    ...
    /**
     * Initiate Window Hash Event Listener
     * @private
     */
    function _initiateHashEventListener() {
        window.addEventListener("hashchange", function(v) {
            var hash = this.location.hash;
            _updateIframeSource(  _formatHashToURL(hash));
        }, false);
    }
